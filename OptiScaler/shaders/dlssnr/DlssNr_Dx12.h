...
class DlssNr_Dx12 : public Shader_Dx12, public DlssNr_Common
{
  ...
  public:
    ...
    // untouched original is taken from it and the edit is written to output alone. Only the output's
    // top-left colour-sized region is read from a colour larger than the output.
    //
    // Returns true only when the resolve wrote the output this frame. Every other way out -- the
    // model still being built, a skipped frame, a failure -- leaves the output exactly as it arrived,
    // and a caller that means to hand the output on in place of the original has to know that.
